...
$display->chart({id=>"scope",row=>8,column=>14,height=>15,width=>50,
              borderStyle=>"shadow",borderColour=>"bold blue",
              title=>"ASCIIloscope Demo",titleColour=>"black on_yellow",
              traces=>[new Trace(traceFile=>"sin.trc") ] }); 

# create a statistics widget
$display->statBox({id=>"stat",row=>26,column=>14,height=>2,width=>50,
...
our %actions=(                  # for keyboard driven actions
   ...
   113=>{ # q exits
     note=>"q = Exit",
     proc=>sub{ printAt(29,10,"Goodbye!");exit;},
   },
...
sub run{
  ...
  if ($key){
    my $OrdKey = ord($key);
    printAt (24,70,"key pressed=$OrdKey");
...
sub menu{
  ...
  # Print Menu...disable by setting $display{showMenu} to zero
    printAt( 8,70, $self->colour("underline")."MENU".$self->colour("reset"),map{$actions{$_}{note} } sort { $a <=> $b } keys %actions);
}
