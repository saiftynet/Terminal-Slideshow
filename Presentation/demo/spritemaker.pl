...
my $canvas;
my $width=$screenParams{width}//45;
my $height=$screenParams{height}//40;
my $dir="../../sprites/";
my $currentFile="";
my $currentSprite="";
my $sprite;
...
$io->{debugCursor}=[($screenParams{top}//4)+18,($screenParams{left}//4)+41];


# setup keyboard short cuts

my $menu=new Term::Graille::Menu(  # no object offered so id will be o0 
          menu=>[["File","New","Load","Save","Delete","Remove","Quit"],
                 ["Edit","Clear","Invert",["Scroll","Left","Right","Up","Down"],["Rotate","CW (90)","(180)","CCW (270)"],["Flip","RL Diagonal","LR Diagonal","Vertical","Horizontal"],["Geometry",["Shrink","Remove Row","Remove Column",],["Grow","Add Row","Add Column",]],],
                 ["Sprites",sort keys %$],
                 "About"],
          redraw=>\&refreshScreen,
          dispatcher=>\&menuActions,
          pos=>[$screenParams{top}-2,$screenParams{left}]
          );
...
my %actions = ( New  =>  sub{ },
                Load =>  sub{ load($dir)},
                Save =>  sub{ save($dir) },
                Clear=>  sub{ $sprite=[map {[("⠀")x8]}(0..3)];refreshScreen();  },
                Invert=>  sub{ $sprite=[map {[("⠀")x8]}(0..3)];refreshScreen();  },
                Rename=> sub{ $sprite=[map {[("⠀")x8]}(0..3)];refreshScreen();  },
                Quit=> sub{ exit;  },
              );
...
sub refreshScreen{
    ...
    $canvas->blockBlit($sprite,$canvas->{width}/4-4,$canvas->{height}/8-1);
	$canvas->draw();
...
sub drawBigSprite{
	...
	for my $c(0..$#{$sprite->[0]}){
	  for my $r (0..$#{$sprite}){
		printAt (($screenParams{top}//0)-2+$r*4,($screenParams{left}//0)+33+$c*2,chr2blk($sprite->[$r]->[$c],"black","white"));
	  }
    }
	# puts the cursor at the desired location on the "BigSprite"
	my ($chX,$chY,$r,$c)=locate(@$cursor);
	printAt (($screenParams{top}//0)+1+4*$#{$sprite}-$cursor->[1],($screenParams{left}//0)+33+$cursor->[0], # overwrite the block
