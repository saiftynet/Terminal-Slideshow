...
sub menuActions{ # dispatcher for menu
	...
	if (exists $actions{$action}){
		...
		printAt($screenPos->[0],$screenPos->[1]+50,$action)
	}
};

our $currentFile="";

my $textarea=new Term::Graille::Textarea(position=>[8,20]);   # no objectId offered so id will be o0

my $canvas=	Term::Graille->new(
		width  => 80,
		height => 50,
		top=>$screenPos->[0]+4, #4
		left=>$screenPos->[1]+10, #8
		borderStyle => "double",
...
sub runPerl{
	system("gnome-terminal -- perl $currentFile");
	$io->start("o1");
}

sub runPython{
	system("gnome-terminal -- python3 $currentFile");
	$io->start("o1");
}
