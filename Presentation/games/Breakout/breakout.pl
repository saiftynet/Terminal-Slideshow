...
my $canvas = Term::Graille->new(
		width  => 148,
		height => 60,
		top=>$screenParams{top}//5,
		left=>$screenParams{left}//20,
		borderStyle => "double",
		title=>"Breakout",
	  );
my $lives=3;
...
$io->{debugCursor}=[$canvas->{top}+17,$canvas->{left}+25];
...
$io->addAction(undef,"q",{note=>"QUIT ",     proc=>sub{
    exit;}}    );
...
sub update{
    ...
    printAt($canvas->{top}-1,$canvas->{left}+30,"Score: $score Level: $level, Lives ".colour("red")."♥ "x$lives.colour("reset") );
}
